...
function pickPeaks(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (currentNum > previousNum && currentNum > nextNum) {
      obj["pos"].push(i);
      obj["peaks"].push(currentNum);
    } else if (currentNum > previousNum && currentNum === nextNum) {
      let index = i;
      while (index < arr.length) {
        let newCurrentNum = arr[index];
        let prevNum = arr[index - 1];
        if (newCurrentNum < currentNum && prevNum === currentNum) {
          if (obj["pos"].includes(i)) break;
          obj["pos"].push(i);
          obj["peaks"].push(currentNum);
        } else if (newCurrentNum > currentNum) {
          break;
        }
        index++;
      }
