...
function zipWith(fn, a0, a1) {
  const shorterArrLength = Math.min(a0.length, a1.length);
  const output = [];

  for (let i = 0; i < shorterArrLength; i++) {
    let currentElemShorter = a0[i];
    let currentElemLonger = a1[i];
    let result = fn(currentElemShorter, currentElemLonger);
