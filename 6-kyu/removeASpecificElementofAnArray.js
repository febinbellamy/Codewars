...
function selectSubarray(arr) {
  ...
  let indexOfElemToRemove = 0;

  while (indexOfElemToRemove < arr.length) {
    let currentElementToRemove = arr[indexOfElemToRemove];
    pairs.push([indexOfElemToRemove, currentElementToRemove]);
  ...
  const subSums = subArrays.map((subArray) => subArray.reduce((a, c) => a + c));
  const subProducts = subArrays.map((subArray) =>
    subArray.reduce((a, c) => a * c)
  );
  const q = [];
  ...
  for (let i = 0; i < subSums.length; i++) {
    if (subSums[i] === 0) {
      ...
      let quotient = Math.abs(subProducts[i] / subSums[i]);
      q.push(quotient);
    }
  }
  const min = Math.min(...q);
  const indexOfMin = q.indexOf(min);
  return pairs[indexOfMin];
}
