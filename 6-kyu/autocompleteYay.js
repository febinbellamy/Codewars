...
function autocomplete(input, dictionary) {
  const filteredInput = input
    ...
    .join("");
  let filteredDictionary = dictionary.filter((element) =>
    element.toLowerCase().startsWith(filteredInput)
  );

  if (filteredDictionary.length > 5) {
    filteredDictionary = filteredDictionary.slice(0, 5);
  } else if (filteredDictionary.length === 0) {
    return [];
  }
