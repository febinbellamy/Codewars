...
function rotate(array, n) {
  // ...
  let count = 0;
  let numberOfRotations = Math.abs(n) % array.length;
  let currentArr = [...array];

  while (count < numberOfRotations) {
    if (n > 0) {
      let lastElement = currentArr.pop();
      let rotatedArr = [lastElement, ...currentArr];
      currentArr = [...rotatedArr];
    } else {
      let firstElement = currentArr.shift();
      let rotatedArr = [...currentArr, firstElement];
      currentArr = [...rotatedArr];
    }
    count++;
  }
