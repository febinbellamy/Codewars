...
function isbnConverter(isbn) {
  let ibnWithoutLastChar = isbn.slice(0, isbn.length - 1);
  let isbnWithNewPrefix = "978-" + ibnWithoutLastChar;
  const digits = [];
  let count = 0;
  for (let i = 0; i < isbnWithNewPrefix.length; i++) {
    let currentChar = isbnWithNewPrefix[i];
    if (/[0-9]/.test(currentChar)) {
      let num = +currentChar * (count % 2 === 0 ? 1 : 3);
      count++;
      digits.push(num);
    }
  }
  const sumOfDigits = digits.reduce((a, c) => a + c, 0);
  const result = sumOfDigits % 10;
  const checkDigit = result === 0 ? 0 : 10 - result;
  return isbnWithNewPrefix + checkDigit;
}
