...
function isValidWalk(walk) {
  ...
  for (let i = 0; i < walk.length; i++) {
    let currentElem = walk[i];

    if (freqObj[currentElem]) {
      freqObj[currentElem]++;
    } else {
      freqObj[currentElem] = 1;
    }
  }
  if (freqObj["n"] || freqObj["s"]) {
    if (freqObj["n"] !== freqObj["s"]) return false;
  }
  if (freqObj["w"] || freqObj["e"]) {
    if (freqObj["e"] !== freqObj["w"]) return false;
  }
