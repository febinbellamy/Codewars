...
function derDieDas(wort) {
  ...
  let vowels = "aeiouäöü";
  const filteredVowels = wort
    .split("")
    .filter((char) => vowels.includes(char.toLowerCase()));
  if (filteredVowels.length < 2) return `das ${wort}`;
  if (filteredVowels.length <= 3) return `die ${wort}`;
  if (filteredVowels.length > 3) return `der ${wort}`;
}
