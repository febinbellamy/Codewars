...
function midtownNav(start, end) {
  ...
  const startAve = parseInt(startSplit[0]);
  const startSt = parseInt(startSplit[4]);

  const endSplit = end.split(" ");
  const endAve = parseInt(endSplit[0]);
  const endSt = parseInt(endSplit[4]);

  return `Walk ${Math.abs(startSt - endSt)} blocks ${
    startSt > endSt ? "south" : "north"
  }, and ${Math.abs(startAve - endAve)} blocks ${
    startAve > endAve ? "east" : "west"
  }`;
}
