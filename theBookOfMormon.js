...
function Mormons(startingNumber, reach, target) {
  ...
  if (startingNumber + reach >= target) return 0;
  let numberOfMormons = startingNumber;
  let numberOfMissions = 0;

  while (numberOfMormons < target) {
    let temp = numberOfMormons;
    numberOfMormons = temp * reach + temp;
    numberOfMissions++;
  }
