...
function cutIt(arr) {
  //coding here...
  let minimumStrLength = arr[0].length;
  for (let i = 1; i < arr.length; i++) {
    let currentStrLength = arr[i].length;
    if (currentStrLength < minimumStrLength) {
      minimumStrLength = currentStrLength;
    }
  }
  return arr.map((word) => word.slice(0, minimumStrLength));
}
