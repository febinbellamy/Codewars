...
function countGrade(scores) {
  const gradesFrequency = {};
  const numberOfSs = scores.filter((grade) => grade === 100).length;
  gradesFrequency["S"] = numberOfSs;
  const numberOfAs = scores.filter(
    (grade) => grade < 100 && grade >= 90
  ).length;
  gradesFrequency["A"] = numberOfAs;
  const numberOfBs = scores.filter((grade) => grade < 90 && grade >= 80).length;
  gradesFrequency["B"] = numberOfBs;
  const numberOfCs = scores.filter((grade) => grade < 80 && grade >= 60).length;
  gradesFrequency["C"] = numberOfCs;
  const numberOfDs = scores.filter((grade) => grade < 60 && grade >= 0).length;
  gradesFrequency["D"] = numberOfDs;
  const numberOfXs = scores.filter((grade) => grade === -1).length;
  gradesFrequency["X"] = numberOfXs;
