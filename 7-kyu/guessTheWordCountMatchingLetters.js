...
function countCorrectCharacters(correctWord, guess) {
  ...
  if (correctWord.length !== guess.length)
    throw new Error("The two words have different lengths");

  let count = 0;
  for (let i = 0; i < guess.length; i++) {
    let currentLetterInCorrectWord = correctWord[i];
    let currentLetterInGuess = guess[i];
    if (currentLetterInCorrectWord === currentLetterInGuess) count++;
  }
  return count;
}
