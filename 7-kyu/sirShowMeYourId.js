...
function showMe(yourID) {
  if (
    yourID[yourID.length - 1] === "-" ||
    yourID[0].toUpperCase() !== yourID[0]
  )
    return false;

  for (let i = 0; i < yourID.length; i++) {
    let currentChar = yourID[i];
    if (currentChar === " " || /[^-a-zA-Z]/.test(currentChar)) return false;
  }
