...
function match(job, candidates) {
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    if (currentCandidateObj["desiresEquity"] && job["equityMax"] === 0) {
      continue;
    }
    for (let j = 0; j < job["locations"].length; j++) {
      let currentJobLocation = job["locations"][j];
      if (
        currentCandidateObj["currentLocation"] === currentJobLocation ||
        currentCandidateObj["desiredLocations"].includes(currentJobLocation)
      ) {
        matches.push(currentCandidateObj);
        break;
      }
    }
  }
