...
function alphabetWar(fight) {
  ...
  for (let i = 0; i < fight.length; i++) {
    let currentLetter = fight[i];

    if (leftSideLetters[currentLetter]) {
      leftSideScore += leftSideLetters[currentLetter];
    } else if (rightSideLetters[currentLetter]) {
      rightSideScore += rightSideLetters[currentLetter];
    }
  }
