...
function elementsSum(arr, d) {
  //your code here
  let sum = 0;
  let indexOfOuterArr = 0;
  let indexOfInnerArr = arr.length - 1;
  while (indexOfOuterArr < arr.length) {
    let currentArr = arr[indexOfOuterArr];
    if (typeof currentArr[indexOfInnerArr] === "undefined") {
      if (d) {
        sum += d;
      }
    } else {
      sum += currentArr[indexOfInnerArr];
    }
    indexOfOuterArr++;
    indexOfInnerArr--;
  }
