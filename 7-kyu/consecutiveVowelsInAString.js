...
function getTheVowels(word) {
  ...
  let vowelIndex = 0;
  let mainIndex = 0;
  let consecutiveVowelCounter = 0;

  while (mainIndex < word.length) {
    let currentChar = word[mainIndex];
    let currentVowelToCheckFor = vowels[vowelIndex];

    if (currentChar === currentVowelToCheckFor) {
      consecutiveVowelCounter++;
      vowelIndex++;
  ...
  return consecutiveVowelCounter;
}
