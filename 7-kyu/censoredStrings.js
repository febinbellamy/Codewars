...
function uncensor(infected, discovered) {
  let indexOfInfected = 0;
  let indexOfDiscovered = 0;
  let newStr = "";

  while (indexOfInfected < infected.length) {
    let currentCharInfected = infected[indexOfInfected];
    let currentCharDiscovered = discovered[indexOfDiscovered];

    if (currentCharInfected === "*") {
      newStr += currentCharDiscovered;
      indexOfDiscovered++;
    } else {
      newStr += currentCharInfected;
    }
    indexOfInfected++;
  }
  return newStr;
