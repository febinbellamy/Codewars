...
function scoreTest(str, right, omit, wrong) {
  ...
  for (let i = 0; i < str.length; i++) {
    let currentScore = str[i];
    if (currentScore in freqCounter) {
      freqCounter[currentScore]++;
    } else {
      freqCounter[currentScore] = 1;
    }
  }
  let totalScore = 0;
  if ("0" in freqCounter && right) {
    totalScore += right * freqCounter["0"];
  }
  if ("1" in freqCounter && omit) {
    totalScore += omit * freqCounter["1"];
  }
  if ("2" in freqCounter && wrong) {
    totalScore -= wrong * freqCounter["2"];
  }
