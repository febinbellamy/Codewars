...
function conjugate(verb) {
  ...
  const prefix = verb.slice(0, verb.length - 2);
  const suffix = verb.slice(verb.length - 2);
  const result = [];
  ...
  for (let i = 0; i < arrToLoopOver.length; i++) {
    let currentConjugation = arrToLoopOver[i];
    result.push(prefix + currentConjugation);
  }
