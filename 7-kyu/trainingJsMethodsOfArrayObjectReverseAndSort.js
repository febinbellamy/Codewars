...
function sortIt(arr) {
  //coding here...
  function populateArr(num, frequency) {
    const result = [];
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (currentNum in freqCounter) {
      freqCounter[currentNum]++;
    } else {
  ...
  for (let key in freqCounter) {
    let frequency = freqCounter[key];
    let number = +key;
    arrOfObjs.push({ number: number, frequency: frequency });
  }
  ...
  for (let j = 0; j < arrOfObjs.length; j++) {
    let num = arrOfObjs[j]["number"];
    let frequency = arrOfObjs[j]["frequency"];
    finalArr.push(...populateArr(num, frequency));
  }
  return finalArr;
}
