...
function bearDollars(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    let currentRelationship = arr[i][1].toLowerCase();
    let currentNumOfJobs = arr[i][0];
    if (rates.hasOwnProperty(currentRelationship)) {
      total += rates[currentRelationship] * currentNumOfJobs;
    } else {
      total += 125 * currentNumOfJobs;
    }
  }
  return total;
}
