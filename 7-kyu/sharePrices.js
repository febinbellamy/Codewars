...
const sharePrice = (invested, changes) => {
  ...
  for (let i = 0; i < changes.length; i++) {
    ...
    let newNum = current$ * (Math.abs(currentChange) / 100);
    // either decrease current$ by the newNum or increase current$ by newNum
    currentChange >= 0 ? (current$ += newNum) : (current$ -= newNum);
  }
  return String(current$.toFixed(2));
};
